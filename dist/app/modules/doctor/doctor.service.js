...
const addSpecialize = async (payload) => {
...
const updateSpecialize = async (id, payload) => {
...
const updateDoctor = async (doctorId, decodedToken, payload) => {
    ...
    try {
        if (Object.keys(updateUser).length) {
            await User.findByIdAndUpdate(existingDoctor.user, { $set: updateUser }, { new: true, runValidators: true, session });
        }
        const updatedDoctor = await Doctor.findByIdAndUpdate(doctorId, { $set: updateDoctorData }, { new: true, runValidators: true, session }).populate("user specialization").exec(); // ✅ populate
        await session.commitTransaction();
        session.endSession();
...
const getAvailableSlots = async (doctorId, date) => {
    ...
    if (!doctor)
        throw new AppError(httpStatusCode.NOT_FOUND, "Doctor not found");
    if (!date)
        throw new AppError(httpStatusCode.NOT_FOUND, "date not found");
